...
import { createLocalInvitations, createLocalOrganization, updateLocalAccount } from "../data/localAccounts";
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (isApiEnabled && pending) {
        ...
        login(session, { remember: true }); sessionStorage.removeItem("spectramind:pending-signup");
      } else if (isApiEnabled) {
        const organization = await createOrganization({ name: organizationName, contactEmail });
        const membership = { organizationId: organization.id, organizationName: organization.name, role: organization.role, onboardingComplete: true };
        ...
        const membership = { organizationId, organizationName: organizationName.trim(), contactEmail: contactEmail.trim(), onboardingComplete: true };
...
function readPending(){try{return JSON.parse(sessionStorage.getItem("spectramind:pending-signup")||"null");}catch{return null;}}
