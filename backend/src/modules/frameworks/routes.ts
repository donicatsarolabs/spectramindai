...
export async function frameworkRoutes(app: FastifyInstance) {
  ...
  app.get("/frameworks", async (request) => {
    ...
    return frameworks.map(framework => ({
      ...framework,
      applicable: isFrameworkApplicable(framework.id, request.user.email),
      availabilityReason: isFrameworkApplicable(framework.id, request.user.email) ? null : "Inapplicable for this CMMC-only client demo",
    }));
  });

  app.get("/organization-frameworks", async (request) => {
    const records = await prisma.organizationFramework.findMany({
      where: { organizationId: request.tenant.organizationId },
      include: { framework: true },
      orderBy: { createdAt: "asc" },
    });
    return records.filter((record) => isFrameworkApplicable(record.frameworkId, request.user.email));
  });
  ...
  app.post("/organization-frameworks", async (request, reply) => {
    ...
    if (!isFrameworkApplicable(frameworkId, request.user.email)) return frameworkInapplicable(reply);
  ...
  app.post("/organization-frameworks/checkout", async (request, reply) => {
    ...
    if (frameworkIds.some(frameworkId => !isFrameworkApplicable(frameworkId, request.user.email))) return frameworkInapplicable(reply);
  ...
  app.get("/controls", async (request) => {
    ...
    if (!isFrameworkApplicable(query.frameworkId, request.user.email)) throw Object.assign(new Error("Framework is inapplicable while CMMC-only demo mode is active"), { statusCode: 403 });
  ...
  app.patch("/controls/:controlId/implementation", async (request, reply) => {
    ...
    if (!isFrameworkApplicable(control.frameworkId, request.user.email)) return frameworkInapplicable(reply);
  ...
  app.get("/dashboard", async (request) => {
    ...
    if (query.frameworkId && !isFrameworkApplicable(query.frameworkId, request.user.email)) throw Object.assign(new Error("Framework is inapplicable while CMMC-only demo mode is active"), { statusCode: 403 });
    const frameworkId = config.CMMC_ONLY_MODE && !hasAllFrameworkAccess(request.user.email) ? CMMC_FRAMEWORK_ID : query.frameworkId;
...
function isFrameworkApplicable(frameworkId: string, email?: string) {
  return !config.CMMC_ONLY_MODE || hasAllFrameworkAccess(email) || frameworkId === CMMC_FRAMEWORK_ID;
}
